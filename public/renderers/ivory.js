...
function renderHome(home, context = {}) {
  ...
  const headline = (current && current.label)
    || (home.now && home.now.value)
    || (home.message && String(home.message).trim())
    || 'Le projet avance.';
  const presentText = (current && current.description)
    || (home.now && home.now.summary)
    || '';
  const progress = timeline && timeline.progress || null;
  const phaseMeta = progress && progress.currentStepLabel && progress.totalSteps
  ...
  const nextDescription = (upcoming && upcoming.description) || (home.next && home.next.description) || '';
...
function fallbackProjectContent() {
  // Temporary editorial fallback for the POC only.
  // It disappears automatically as soon as Studio / Compiler publish
  // semantic `content.project` data. No layout decision is encoded here.
  return {
    intro: {
    ...
    sections: [
      {
        type: 'focus',
        title: 'Un projet pensé à partir des usages.',
        body: 'Les espaces sont conçus à partir des activités réellement réalisées au cours d’une journée : se concentrer, échanger, collaborer, se réunir ou simplement changer de rythme.'
      },
      {
        type: 'keyFigures',
        title: 'Quelques repères',
        items: [
          { value: '1 200', label: 'm² d’espaces' },
          { value: '90', label: 'collaborateurs' },
          { value: '8', label: 'espaces collaboratifs' }
        ]
      },
      {
        type: 'text',
        title: 'Des espaces pour plusieurs manières de travailler.',
        body: 'L’objectif n’est pas d’imposer une seule façon de faire, mais d’offrir des environnements adaptés aux différents moments de la journée. Les choix d’aménagement cherchent à rendre ces possibilités immédiatement lisibles et faciles à utiliser.'
      },
      { type: 'timeline' },
      {
        type: 'quote',
        quote: 'Nous voulons créer un lieu où chacun puisse trouver l’environnement adapté à ce qu’il a à faire.',
      ...
      },
      {
        ...
        items: [
          { title: 'Partir des usages', body: 'Observer le travail réel avant de figer les réponses spatiales.' },
          { title: 'Donner le choix', body: 'Permettre de changer d’environnement selon l’activité et le moment.' },
          { title: 'Préparer l’évolution', body: 'Concevoir des espaces capables de s’adapter dans le temps.' }
        ]
      },
      { type: 'team' }
    ]
...
function renderProject(project, context = {}) {
  const fallback = fallbackProjectContent();
  const source = project && typeof project === 'object' ? project : {};
  const intro = source.intro && typeof source.intro === 'object' ? source.intro : fallback.intro;
